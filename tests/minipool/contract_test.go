...
func TestDetails(t *testing.T) {
	...
	networkNodeFee, err := network.GetNodeFee(ggp, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if status, err := mp.GetStatusDetails(nil); err != nil {
		...
		if status.Status != ggptypes.Withdrawable {
	...
	if depositType, err := mp.GetDepositType(nil); err != nil {
		t.Error(err)
	} else if depositType != ggptypes.Full {
	...
	if node, err := mp.GetNodeDetails(nil); err != nil {
		...
		if node.DepositBalance.Cmp(avax.EthToWei(16)) != 0 {
			t.Errorf("Incorrect minipool node deposit balance %s", node.DepositBalance.String())
		}
		if node.RefundBalance.Cmp(avax.EthToWei(16)) != 0 {
	...
	if user, err := mp.GetUserDetails(nil); err != nil {
		...
		if user.DepositBalance.Cmp(avax.EthToWei(16)) != 0 {
	...
	if withdrawalCredentials, err := minipool.GetMinipoolWithdrawalCredentials(ggp, mp.Address, nil); err != nil {
		t.Error(err)
	} else {
...
func TestStake(t *testing.T) {
	...
	if status, err := mp.GetStatus(nil); err != nil {
		t.Error(err)
	} else if status != ggptypes.Prelaunch {
	...
	if status, err := mp.GetStatus(nil); err != nil {
		t.Error(err)
	} else if status != ggptypes.Staking {
...
func TestDissolve(t *testing.T) {
	...
	if status, err := mp.GetStatus(nil); err != nil {
		t.Error(err)
	} else if status != ggptypes.Initialized {
		t.Errorf("Incorrect initial minipool status %s", status.String())
	}
	...
	if status, err := mp.GetStatus(nil); err != nil {
		t.Error(err)
	} else if status != ggptypes.Dissolved {
		t.Errorf("Incorrect updated minipool status %s", status.String())
	}
...
func TestClose(t *testing.T) {
	...
	if exists, err := minipool.GetMinipoolExists(ggp, mp.Address, nil); err != nil {
		t.Error(err)
	} else if !exists {
	...
	hash, err := avax.SendTransaction(ggp.Client, mp.Address, big.NewInt(1337), opts) // Ganache's default chain ID is 1337
	if err != nil {
		t.Errorf("Error sending ETH to minipool: %s", err.Error())
	}
	utils.WaitForTransaction(ggp.Client, hash)
...
func TestWithdrawValidatorBalance(t *testing.T) {
	...
	// Make user deposit
	userDepositAmount := avax.EthToWei(16)
	...
	rethContractBalance1, err := tokens.GetRETHContractETHBalance(ggp, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	opts.Value = avax.EthToWei(32)
	if _, err := mp.Contract.Transfer(opts); err != nil {
		t.Fatal(err)
	}

	// Get node balances before withdrawal
	nodeBalance1, err := tokens.GetBalances(ggp, nodeAccount.Address, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if nodeBalance2, err := tokens.GetBalances(ggp, nodeAccount.Address, nil); err != nil {
		t.Fatal(err)
	...
	if rethContractBalance2, err := tokens.GetRETHContractETHBalance(ggp, nil); err != nil {
		t.Fatal(err)
	...
	if rethTotalCollateral, err := tokens.GetRETHTotalCollateral(ggp, nil); err != nil {
		t.Fatal(err)
	...
	if rethCollateralRate, err := tokens.GetRETHCollateralRate(ggp, nil); err != nil {
		t.Fatal(err)
